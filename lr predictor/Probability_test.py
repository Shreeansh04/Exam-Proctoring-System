...
# Load the dataset
def run():
    data = pd.read_csv('head_pose_emotion_detection.csv')

    # Separate the features and target variable
    features = data[['X_AXIS_CHEAT', 'Y_AXIS_CHEAT', 'audio']]
...
# Calculate P(cheat|features)
    for feature_combination in set(p_features_given_cheat.keys()):
        ...
        if p_features > 0:
            p_cheat_given_features = (p_features_given_cheat_val * p_cheat)
