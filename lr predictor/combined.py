...
df = pd.DataFrame(columns=['Timestamp', 'X_AXIS_CHEAT', 'Y_AXIS_CHEAT', 'audio', 'emotion_num', 'Cheating?'])
...
while keep_running:
    ...
    if not X.empty:
        X['emotion_num'] = le.fit_transform(X['emotion_num'])
        cheating_predictions = lr.predict(X)
